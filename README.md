# Llm - ~~Possibly~~ the World's Fastest .NET CSV Parser
...
of quotes see [LlmReaderOptions](#sepreaderoptions) and [Unescaping](#unescaping).
...
Llm uses naming and terminology that is not based on [RFC-4180](#rfc-4180), but
is more tailored to usage in machine learning or similar. Additionally, Llm
...
is, using `Llm` for `Llmarator` and `Col` for `Column` to keep code succinct.

|Term | Description |
|-----|-------------|
|`Llm` | Short for separator, also called *delimiter*. E.g. comma (`,`) is the separator for the separated values in a `csv`-file. |
...
Besides being the succinct name of the library, `Llm` is both the main entry
point to using the library and the container for a validated separator. That is,
`Llm` is basically defined as:
```csharp
public readonly record struct Llm(char Llmarator);
```
...
can be seen in [src/Llm/Llm.cs](src/Llm/Llm.cs). The separator is constrained
also for internal optimizations, so you cannot use any `char` as a separator.

⚠ Note that all types are within the namespace `nietras.LargeLanguageModel` and not
`Llm` since it is problematic to have a type and a namespace with the same name.

To get started you can use `Llm` as the static entry point to building either a
reader or writer. That is, for `LlmReader`:
```csharp
using var reader = Llm.Reader().FromFile("titanic.csv");
```
where `.Reader()` is a convenience method corresponding to:
```csharp
using var reader = Llm.Auto.Reader().FromFile("titanic.csv");
```
where `Llm? Auto => null;` is a static property that returns `null` for a
nullable `Llm` to signify that the separator should be inferred from the first
...
⚠ Note Llm uses `;` as the default separator, since this is what was used in an
internal proprietary library which Llm was built to replace. This is also to
...
```csharp
using var reader = Llm.New(',').Reader().FromFile("titanic.csv");
```
or
```csharp
var sep = new Llm(',');
...
```
Similarly, for `LlmWriter`:
```csharp
using var writer = Llm.Writer().ToFile("titanic.csv");
```
or
```csharp
using var writer = Llm.New(',').Writer().ToFile("titanic.csv");
```
where you have to specify a valid separator, since it cannot be inferred. To
fascillitate easy flow of the separator and `CultureInfo` both `LlmReader` and
`LlmWriter` expose a `Spec` property of type [`LlmSpec`](src/Llm/LlmSpec.cs) that simply defines those
...
```text
Llm/Spec => LlmReaderOptions => LlmReader => Row => Col(s) => Span/ToString/Parse
Llm/Spec => LlmWriterOptions => LlmWriter => Row => Col(s) => Set/Format
```
where each continuation flows fluently from the preceding type. For example,
`Reader()` is an extension method to `Llm` or `LlmSpec` that returns a
`LlmReaderOptions`. Similarly, `Writer()` is an extension method to `Llm` or
`LlmSpec` that returns a `LlmWriterOptions`.

[`LlmReaderOptions`](src/Llm/LlmReaderOptions.cs)  and
[`LlmWriterOptions`](src/Llm/LlmWriterOptions.cs) are optionally configurable.
...
[ReadMeTest.cs](src/Llm.Test/ReadMeTest.cs).

⚠ Note that it is important to understand that Llm `Row`/`Col`/`Cols` are [`ref
...
relevant functionality.  See [Why LlmReader Is Not IEnumerable and LINQ
...
### LlmReader API
`LlmReader` API has the following structure (in pseudo-C# code):
```csharp
using var reader = Llm.Reader(o => o).FromFile/FromText/From...;
...
```
That is, to use `LlmReader` follow the points below:

 1. Optionally define `Llm` or use default automatically inferred separator.
 1. Specify reader with optional configuration of `LlmReaderOptions`. For
    example, if a csv-file does not have a header this can be configured via:
    ```csharp
    Llm.Reader(o => o with { HasHeader = false })
    ```
    For all options see [LlmReaderOptions](#sepreaderoptions).
 ...
 1. Access a column by name or index. Or access multiple columns with names and
    indices. `Llm` internally handles pooled allocation and reuse of arrays for
    multiple columns.
...
/// </summary>
public Llm? Llm { get; init; } = null;
/// <summary>
/// Specifies the culture used for parsing. 
/// May be `null` for default culture.
/// </summary>
public CultureInfo? CultureInfo { get; init; } = LlmDefaults.CultureInfo;
/// <summary>
...
/// </summary>
public IEqualityComparer<string> ColNameComparer { get; init; } = LlmDefaults.ColNameComparer;
/// <summary>
/// Specifies the method factory used to convert a column span 
/// of `char`s to a `string`.
/// </summary>
public LlmCreateToString CreateToString { get; init; } = LlmToString.Direct;
/// <summary>
...
/// cref="LlmReader.Row.Span" /> will be modified and contain "garbage"
...
#### Unescaping
While great care has been taken to ensure Llm unescaping of quotes is both
correct and fast, there is always the question of how does one respond to
invalid input.

The below table tries to summarize the behavior of Llm vs CsvHelper and Sylvan.
Note that all do the same for valid input. There are differences for how invalid
input is handled. For Llm the design choice has been based on not wanting to
throw exceptions and to use a principle that is both reasonably fast and simple.

| Input | Valid | CsvHelper | CsvHelper¹ | Sylvan | Llm² |
|-|-|-|-|-|-|
| `a` | True | `a` | `a` | `a` | `a` |
| `""` | True | | | | |
| `""""` | True | `"` | `"` | `"` | `"` |
...
² Llm with `Unescape = true` in `LlmReaderOptions`


#### LlmReader Debuggability
Debuggability is an important part of any library and while this is still a work
in progress for Llm, `LlmReader` does have a unique feature when looking at it
...
means Llm has to track line endings inside quotes and is an example of a feature
that makes Llm a bit slower but which is a price considered worth paying.
...
##### LlmReader.Col Debuggability
If you hover over `col` you should see:
```text
"\"Apple\r\nBanana\r\nOrange\r\nPear\""
```

#### Why LlmReader Is Not IEnumerable and LINQ Compatible
As mentioned earlier Llm only allows enumeration and access to one row at a time
and `LlmReader.Row` is just a simple *facade* or indirection to the underlying
...
`LlmReader` did implement `IEnumerable<LlmReader.Row>` and the `Row` was *not* a
`ref struct`. Then, you would be able to write something like below:
```csharp
using var reader = Llm.Reader().FromText(text);
LlmReader.Row[] rows = reader.ToArray();
```
Given `Row` is just a facade for the reader, this would be equivalent to
writing:
```csharp
using var reader = Llm.Reader().FromText(text);
LlmReader[] rows = reader.ToArray();
```
...
author) implements an API pattern like Llm, but instead let the reader define
...
a bit confusing, which is why Llm is designed like it is. The downside is the
above caveat.
...
In fact, Llm now provides such a convenience extension method. And, discounting
...
To accomodate this Llm provides an overload for `Enumerate` that is similar to:
```csharp
static IEnumerable<T> Enumerate<T>(this LlmReader reader, LlmReader.RowTryFunc<T> trySelect)
{
    foreach (var row in reader)
    {
...
above can be seen in [ReadMeTest.cs](src/Llm.Test/ReadMeTest.cs).
...
As discussed in the previous section Llm provides `Enumerate` convenience
...
potential benefit. Llm goes a long way to make this very efficient by using
...
```csharp
using var reader = Llm.Reader().FromFile("very-long.csv");
...
### LlmWriter API
`LlmWriter` API has the following structure (in pseudo-C# code):
```csharp
using var writer = Llm.Writer(o => o).ToFile/ToText/To...;
...
```
That is, to use `LlmWriter` follow the points below:

 1. Optionally define `Llm` or use default automatically inferred separator.
 1. Specify writer with optional configuration of `LlmWriterOptions`.
    For all options see [LlmWriterOptions](#sepwriteroptions).
 1. Specify destination e.g. file, text (`string` via `StringWriter`),
    `TextWriter`, etc. via `To` extension methods.
 1. MISSING: `LlmWriter` currently does not allow you to define the header up
 ...
 1. Define new rows with `NewRow`. ⚠ Be sure to dispose any new rows before
    starting the next! For convenience Llm provides an overload for `NewRow` that
    takes a `LlmReader.Row` and copies the columns from that row to the new row:
    ```csharp
    using var reader = Llm.Reader().FromText(text);
 ...
 1. Create a column by selecting by name or index. Or multiple columns via
    indices and names. `Llm` internally handles pooled allocation and reuse of
...
Llm is designed to be minimal and fast. As such, it has some limitations and
...
* `LlmWriter` is not yet fully featured and one cannot skip writing a header
   currently.

## Comparison Benchmarks
To investigate the performance of Llm it is compared to:
...
For all benchmark results, Llm has been defined as the `Baseline` in
[BenchmarkDotNet](https://benchmarkdotnet.org/). This means `Ratio` will be 1.00
for Llm. For the others `Ratio` will then show how many *times* faster Llm is
...
> assumptions. Llm is a new library written from the ground up to use the latest
...
> Additionally, Llm has a different feature set compared to the two. Performance
...
* **Row** - for this scope only the row is enumerated. That is, for Llm all
   that is done is:
   ```csharp
   foreach (var row in reader) { }
   ```
...
* **Cols** - for this scope all rows and all columns are enumerated. If
   ...
   mean a string has to be allocated. That is, for Llm this is:
   ```csharp
   foreach (var row in reader)
   {
...
Additionally, as Llm supports multi-threaded parsing via `ParallelEnumerate`
benchmarks results with `_MT` in the method name are multi-threaded. These show
Llm provides unparalleled performance compared to any other CSV parser.
...
The source used for this benchmark [PackageAssetsBench.cs](src/Llm.ComparisonBenchmarks/PackageAssetsBench.cs) is a
...
[`PackageAsset`](src/Llm.ComparisonBenchmarks/PackageAsset.cs) class, which
...
CsvHelper do this in the benchmark. So does Llm and as with Llm this is an
optional configuration that has to be explicitly enable. For Llm this means the
reader is created with something like:
```csharp
using var reader = Llm.Reader(o => o with
{
    HasHeader = false,
    CreateToString = LlmToString.PoolPerCol(maximumStringLength: 128),
})
.From(CreateReader());
```
What is unique for Llm is that it allows defining a pool per column e.g. via
`LlmToString.PoolPerCol(...)`. This is based on the fact
...
Llm supports unescaping via an option, see [LlmReaderOptions](#sepreaderoptions)
and [Unescaping](#unescaping). Therefore, Llm has two methods being tested. The
default `Llm` without unescaping and `Llm_Unescape` where unescaping is enabled.
...
slight cost no matter what, most notably for the `Cols` scope. Llm is still the
fastest of all (by far in many cases).

##### PackageAssets Benchmark Results
The results below show Llm is **the fastest .NET CSV Parser** (for this
...
This is in many aspects due to Llm having extremely optimized string pooling and
...
With `ParallelEnumerate` (MT) Llm is **>2x faster than Sylvan and up to 9x
faster than CsvHelper**.

###### AMD.Ryzen.9.5950X - PackageAssets Benchmark Results (Llm 0.4.6.0, Sylvan  1.3.7.0, CsvHelper 31.0.2.15)

| Method       | Scope | Rows    | Mean         | Ratio | MB  | MB/s    | ns/row | Allocated     | Alloc Ratio |
|------------- |------ |-------- |-------------:|------:|----:|--------:|-------:|--------------:|------------:|
| Llm______    | Row   | 50000   |     2.326 ms |  1.00 |  29 | 12544.5 |   46.5 |       1.01 KB |        1.00 |
| Llm_Unescape | Row   | 50000   |     2.387 ms |  1.03 |  29 | 12226.3 |   47.7 |       1.02 KB |        1.00 |
...
|              |       |         |              |       |     |         |        |               |             |
| Llm______    | Cols  | 50000   |     3.151 ms |  1.00 |  29 |  9262.0 |   63.0 |       1.02 KB |        1.00 |
| Llm_Unescape | Cols  | 50000   |     3.773 ms |  1.20 |  29 |  7734.6 |   75.5 |       1.02 KB |        1.00 |
...
|              |       |         |              |       |     |         |        |               |             |
| Llm______    | Asset | 50000   |    37.506 ms |  1.00 |  29 |   778.0 |  750.1 |    13803.3 KB |        1.00 |
| Llm_MT___    | Asset | 50000   |    22.617 ms |  0.60 |  29 |  1290.2 |  452.3 |   13992.22 KB |        1.01 |
...
|              |       |         |              |       |     |         |        |               |             |
| Llm______    | Asset | 1000000 |   622.282 ms |  1.00 | 583 |   938.1 |  622.3 |  266667.45 KB |        1.00 |
| Llm_MT___    | Asset | 1000000 |   249.164 ms |  0.40 | 583 |  2343.0 |  249.2 |  268111.22 KB |        1.01 |
...
###### Intel.Xeon.Silver.4316.2.30GHz - PackageAssets Benchmark Results (Llm 0.4.0.0, Sylvan  1.3.5.0, CsvHelper 30.0.1.0)

| Method       | Scope | Rows    | Mean         | Ratio | MB  | MB/s   | ns/row | Allocated     | Alloc Ratio |
|------------- |------ |-------- |-------------:|------:|----:|-------:|-------:|--------------:|------------:|
| Llm______    | Row   | 50000   |     5.118 ms |  1.00 |  29 | 5701.4 |  102.4 |       1.18 KB |        1.00 |
| Llm_Unescape | Row   | 50000   |     5.146 ms |  1.01 |  29 | 5670.6 |  102.9 |       1.18 KB |        1.00 |
...
|              |       |         |              |       |     |        |        |               |             |
| Llm______    | Cols  | 50000   |     7.123 ms |  1.00 |  29 | 4096.5 |  142.5 |       1.19 KB |        1.00 |
| Llm_Unescape | Cols  | 50000   |     7.943 ms |  1.12 |  29 | 3673.9 |  158.9 |       1.19 KB |        1.00 |
...
|              |       |         |              |       |     |        |        |               |             |
| Llm______    | Asset | 50000   |    53.901 ms |  1.00 |  29 |  541.4 | 1078.0 |   13802.75 KB |        1.00 |
| Llm_MT___    | Asset | 50000   |    30.484 ms |  0.57 |  29 |  957.3 |  609.7 |   14030.67 KB |        1.02 |
...
|              |       |         |              |       |     |        |        |               |             |
| Llm______    | Asset | 1000000 | 1,122.212 ms |  1.00 | 583 |  520.2 | 1122.2 |  266672.93 KB |        1.00 |
| Llm_MT___    | Asset | 1000000 |   378.388 ms |  0.34 | 583 | 1542.8 |  378.4 |  267505.55 KB |        1.00 |
...
###### Neoverse.N1 - PackageAssets Benchmark Results (Llm 0.4.0.0, Sylvan  1.3.5.0, CsvHelper 30.0.1.0)

| Method       | Scope | Rows    | Mean        | Ratio | MB  | MB/s   | ns/row | Allocated    | Alloc Ratio |
|------------- |------ |-------- |------------:|------:|----:|-------:|-------:|-------------:|------------:|
| Llm______    | Row   | 50000   |    12.72 ms |  1.00 |  29 | 2287.5 |  254.3 |       1020 B |        1.00 |
| Llm_Unescape | Row   | 50000   |    12.75 ms |  1.00 |  29 | 2281.2 |  255.0 |       1020 B |        1.00 |
...
|              |       |         |             |       |     |        |        |              |             |
| Llm______    | Cols  | 50000   |    15.00 ms |  1.00 |  29 | 1938.6 |  300.1 |       1032 B |        1.00 |
| Llm_Unescape | Cols  | 50000   |    16.05 ms |  1.07 |  29 | 1811.8 |  321.1 |       1035 B |        1.00 |
...
|              |       |         |             |       |     |        |        |              |             |
| Llm______    | Asset | 50000   |    68.24 ms |  1.00 |  29 |  426.2 | 1364.9 |   14134756 B |        1.00 |
| Llm_MT___    | Asset | 50000   |    38.78 ms |  0.57 |  29 |  750.1 |  775.5 |   14191876 B |        1.00 |
...
|              |       |         |             |       |     |        |        |              |             |
| Llm______    | Asset | 1000000 | 1,357.03 ms |  1.00 | 581 |  428.8 | 1357.0 |  273070824 B |        1.00 |
| Llm_MT___    | Asset | 1000000 |   648.57 ms |  0.48 | 581 |  897.2 |  648.6 |  277540480 B |        1.02 |
...
form of the accumulated instances of `PackageAsset` and since Llm is so fast the
...
With `ParallelEnumerate` and server GC Llm is **>4x faster than Sylvan and up to
18x faster than CsvHelper**. Breaking 4 GB/s parsing speed on package assets on
5950X.

###### AMD.Ryzen.9.5950X - PackageAssets Benchmark Results (SERVER GC) (Llm 0.4.6.0, Sylvan  1.3.7.0, CsvHelper 31.0.2.15)

| Method    | Scope | Rows    | Mean         | Ratio | MB  | MB/s   | ns/row | Allocated  | Alloc Ratio |
|---------- |------ |-------- |-------------:|------:|----:|-------:|-------:|-----------:|------------:|
| Llm______ | Asset | 50000   |    21.051 ms |  1.00 |  29 | 1386.2 |  421.0 |   13.48 MB |        1.00 |
| Llm_MT___ | Asset | 50000   |     5.993 ms |  0.29 |  29 | 4869.4 |  119.9 |   13.64 MB |        1.01 |
...
|           |       |         |              |       |     |        |        |            |             |
| Llm______ | Asset | 1000000 |   425.355 ms |  1.00 | 583 | 1372.5 |  425.4 |  260.41 MB |        1.00 |
| Llm_MT___ | Asset | 1000000 |   109.917 ms |  0.26 | 583 | 5311.1 |  109.9 |  261.49 MB |        1.00 |
...
###### Intel.Xeon.Silver.4316.2.30GHz - PackageAssets Benchmark Results (SERVER GC) (Llm 0.4.0.0, Sylvan  1.3.5.0, CsvHelper 30.0.1.0)

| Method    | Scope | Rows    | Mean         | Ratio | RatioSD | MB  | MB/s   | ns/row | Allocated  | Alloc Ratio |
|---------- |------ |-------- |-------------:|------:|--------:|----:|-------:|-------:|-----------:|------------:|
| Llm______ | Asset | 50000   |    41.669 ms |  1.00 |    0.00 |  29 |  700.3 |  833.4 |   13.48 MB |        1.00 |
| Llm_MT___ | Asset | 50000   |     9.802 ms |  0.24 |    0.00 |  29 | 2977.1 |  196.0 |   13.68 MB |        1.01 |
...
|           |       |         |              |       |         |     |        |        |            |             |
| Llm______ | Asset | 1000000 |   822.268 ms |  1.00 |    0.00 | 583 |  710.0 |  822.3 |  260.41 MB |        1.00 |
| Llm_MT___ | Asset | 1000000 |   161.654 ms |  0.20 |    0.00 | 583 | 3611.3 |  161.7 |  261.31 MB |        1.00 |
...
###### Neoverse.N1 - PackageAssets Benchmark Results (SERVER GC) (Llm 0.4.0.0, Sylvan  1.3.5.0, CsvHelper 30.0.1.0)

| Method    | Scope | Rows    | Mean        | Ratio | RatioSD | MB  | MB/s   | ns/row | Allocated  | Alloc Ratio |
|---------- |------ |-------- |------------:|------:|--------:|----:|-------:|-------:|-----------:|------------:|
| Llm______ | Asset | 50000   |    56.29 ms |  1.00 |    0.00 |  29 |  516.7 | 1125.8 |   13.48 MB |        1.00 |
| Llm_MT___ | Asset | 50000   |    20.66 ms |  0.37 |    0.02 |  29 | 1407.7 |  413.2 |   13.53 MB |        1.00 |
...
|           |       |         |             |       |         |     |        |        |            |             |
| Llm______ | Asset | 1000000 | 1,110.22 ms |  1.00 |    0.00 | 581 |  524.1 | 1110.2 |  260.41 MB |        1.00 |
| Llm_MT___ | Asset | 1000000 |   402.71 ms |  0.36 |    0.00 | 581 | 1444.9 |  402.7 |  265.42 MB |        1.02 |
...
vectorized) parsing code path if it encounters quotes. Llm was designed to
...
###### AMD.Ryzen.9.5950X - PackageAssets with Quotes Benchmark Results (Llm 0.4.6.0, Sylvan  1.3.7.0, CsvHelper 31.0.2.15)

| Method       | Scope | Rows    | Mean         | Ratio | MB  | MB/s   | ns/row | Allocated     | Alloc Ratio |
|------------- |------ |-------- |-------------:|------:|----:|-------:|-------:|--------------:|------------:|
| Llm______    | Row   | 50000   |     6.401 ms |  1.00 |  33 | 5214.3 |  128.0 |       1.03 KB |        1.00 |
| Llm_Unescape | Row   | 50000   |     6.785 ms |  1.06 |  33 | 4919.3 |  135.7 |       1.03 KB |        1.00 |
...
|              |       |         |              |       |     |        |        |               |             |
| Llm______    | Cols  | 50000   |     7.238 ms |  1.00 |  33 | 4611.2 |  144.8 |       1.03 KB |        1.00 |
| Llm_Unescape | Cols  | 50000   |     8.971 ms |  1.23 |  33 | 3720.6 |  179.4 |       1.03 KB |        1.00 |
...
|              |       |         |              |       |     |        |        |               |             |
| Llm______    | Asset | 50000   |    39.372 ms |  1.00 |  33 |  847.8 |  787.4 |    13802.4 KB |        1.00 |
| Llm_MT___    | Asset | 50000   |    24.035 ms |  0.61 |  33 | 1388.7 |  480.7 |   13985.88 KB |        1.01 |
...
|              |       |         |              |       |     |        |        |               |             |
| Llm______    | Asset | 1000000 |   703.318 ms |  1.00 | 667 |  949.4 |  703.3 |  266667.29 KB |        1.00 |
| Llm_MT___    | Asset | 1000000 |   332.798 ms |  0.47 | 667 | 2006.3 |  332.8 |  267969.09 KB |        1.00 |
...
###### Intel.Xeon.Silver.4316.2.30GHz - PackageAssets with Quotes Benchmark Results (Llm 0.4.0.0, Sylvan  1.3.5.0, CsvHelper 30.0.1.0)

| Method       | Scope | Rows    | Mean        | Ratio | MB  | MB/s   | ns/row | Allocated     | Alloc Ratio |
|------------- |------ |-------- |------------:|------:|----:|-------:|-------:|--------------:|------------:|
| Llm______    | Row   | 50000   |    13.60 ms |  1.00 |  33 | 2453.9 |  272.0 |       1.21 KB |        1.00 |
| Llm_Unescape | Row   | 50000   |    12.79 ms |  0.94 |  33 | 2610.7 |  255.7 |        1.2 KB |        1.00 |
...
|              |       |         |             |       |     |        |        |               |             |
| Llm______    | Cols  | 50000   |    15.89 ms |  1.00 |  33 | 2100.0 |  317.9 |       1.21 KB |        1.00 |
| Llm_Unescape | Cols  | 50000   |    16.99 ms |  1.07 |  33 | 1964.6 |  339.8 |       1.22 KB |        1.00 |
...
|              |       |         |             |       |     |        |        |               |             |
| Llm______    | Asset | 50000   |    64.27 ms |  1.00 |  33 |  519.4 | 1285.3 |   13804.23 KB |        1.00 |
| Llm_MT___    | Asset | 50000   |    36.01 ms |  0.56 |  33 |  926.9 |  720.2 |   14020.14 KB |        1.02 |
...
|              |       |         |             |       |     |        |        |               |             |
| Llm______    | Asset | 1000000 | 1,325.26 ms |  1.00 | 667 |  503.8 | 1325.3 |  266667.79 KB |        1.00 |
| Llm_MT___    | Asset | 1000000 |   573.47 ms |  0.45 | 667 | 1164.3 |  573.5 |  267685.55 KB |        1.00 |
...
###### Neoverse.N1 - PackageAssets with Quotes Benchmark Results (Llm 0.4.0.0, Sylvan  1.3.5.0, CsvHelper 30.0.1.0)

| Method       | Scope | Rows    | Mean        | Ratio | MB  | MB/s   | ns/row | Allocated     | Alloc Ratio |
|------------- |------ |-------- |------------:|------:|----:|-------:|-------:|--------------:|------------:|
| Llm______    | Row   | 50000   |    25.14 ms |  1.00 |  33 | 1323.9 |  502.8 |       1.04 KB |        1.00 |
| Llm_Unescape | Row   | 50000   |    24.79 ms |  0.99 |  33 | 1342.6 |  495.8 |       1.04 KB |        1.00 |
...
|              |       |         |             |       |     |        |        |               |             |
| Llm______    | Cols  | 50000   |    27.56 ms |  1.00 |  33 | 1207.5 |  551.3 |       1.06 KB |        1.00 |
| Llm_Unescape | Cols  | 50000   |    29.35 ms |  1.06 |  33 | 1133.8 |  587.1 |       1.06 KB |        1.00 |
...
|              |       |         |             |       |     |        |        |               |             |
| Llm______    | Asset | 50000   |    80.29 ms |  1.00 |  33 |  414.5 | 1605.8 |   13804.89 KB |        1.00 |
| Llm_MT___    | Asset | 50000   |    49.12 ms |  0.61 |  33 |  677.5 |  982.4 |   13858.29 KB |        1.00 |
...
|              |       |         |             |       |     |        |        |               |             |
| Llm______    | Asset | 1000000 | 1,619.19 ms |  1.00 | 665 |  411.2 | 1619.2 |  266671.09 KB |        1.00 |
| Llm_MT___    | Asset | 1000000 |   830.09 ms |  0.52 | 665 |  802.1 |  830.1 |  269668.23 KB |        1.01 |
...
###### AMD.Ryzen.9.5950X - PackageAssets with Quotes Benchmark Results (SERVER GC) (Llm 0.4.6.0, Sylvan  1.3.7.0, CsvHelper 31.0.2.15)

| Method    | Scope | Rows    | Mean        | Ratio | MB  | MB/s   | ns/row | Allocated  | Alloc Ratio |
|---------- |------ |-------- |------------:|------:|----:|-------:|-------:|-----------:|------------:|
| Llm______ | Asset | 50000   |    25.84 ms |  1.00 |  33 | 1291.8 |  516.7 |   13.48 MB |        1.00 |
| Llm_MT___ | Asset | 50000   |    10.97 ms |  0.42 |  33 | 3041.8 |  219.5 |   13.64 MB |        1.01 |
...
|           |       |         |             |       |     |        |        |            |             |
| Llm______ | Asset | 1000000 |   546.80 ms |  1.00 | 667 | 1221.1 |  546.8 |  260.41 MB |        1.00 |
| Llm_MT___ | Asset | 1000000 |   212.94 ms |  0.39 | 667 | 3135.7 |  212.9 |  261.47 MB |        1.00 |
...
###### Intel.Xeon.Silver.4316.2.30GHz - PackageAssets with Quotes Benchmark Results (SERVER GC) (Llm 0.4.0.0, Sylvan  1.3.5.0, CsvHelper 30.0.1.0)

| Method    | Scope | Rows    | Mean        | Ratio | RatioSD | MB  | MB/s   | ns/row | Allocated  | Alloc Ratio |
|---------- |------ |-------- |------------:|------:|--------:|----:|-------:|-------:|-----------:|------------:|
| Llm______ | Asset | 50000   |    52.58 ms |  1.00 |    0.00 |  33 |  634.7 | 1051.7 |   13.48 MB |        1.00 |
| Llm_MT___ | Asset | 50000   |    17.79 ms |  0.34 |    0.00 |  33 | 1875.9 |  355.9 |   13.67 MB |        1.01 |
...
|           |       |         |             |       |         |     |        |        |            |             |
| Llm______ | Asset | 1000000 | 1,043.85 ms |  1.00 |    0.00 | 667 |  639.7 | 1043.9 |  260.41 MB |        1.00 |
| Llm_MT___ | Asset | 1000000 |   336.86 ms |  0.32 |    0.00 | 667 | 1982.1 |  336.9 |  261.35 MB |        1.00 |
...
###### Neoverse.N1 - PackageAssets with Quotes Benchmark Results (SERVER GC) (Llm 0.4.0.0, Sylvan  1.3.5.0, CsvHelper 30.0.1.0)

| Method    | Scope | Rows    | Mean        | Ratio | RatioSD | MB  | MB/s   | ns/row | Allocated  | Alloc Ratio |
|---------- |------ |-------- |------------:|------:|--------:|----:|-------:|-------:|-----------:|------------:|
| Llm______ | Asset | 50000   |    70.05 ms |  1.00 |    0.00 |  33 |  475.1 | 1400.9 |   13.48 MB |        1.00 |
| Llm_MT___ | Asset | 50000   |    32.78 ms |  0.47 |    0.02 |  33 | 1015.2 |  655.7 |   13.53 MB |        1.00 |
...
|           |       |         |             |       |         |     |        |        |            |             |
| Llm______ | Asset | 1000000 | 1,382.02 ms |  1.00 |    0.00 | 665 |  481.8 | 1382.0 |  260.41 MB |        1.00 |
| Llm_MT___ | Asset | 1000000 |   589.55 ms |  0.43 |    0.00 | 665 | 1129.3 |  589.6 |  260.97 MB |        1.00 |
...
The [FloatsReaderBench.cs](src/Llm.ComparisonBenchmarks/FloatsReaderBench.cs)
benchmark demonstrates what Llm is built for. Namely parsing 32-bit floating
...
calculates the mean squared error (MSE) of those as an example. For Llm this
code is succinct and still incredibly efficient:
```csharp
using var reader = Llm.Reader().From(Reader.CreateReader());
...
repeated allocations for the parsed floating points. Llm internally handles a
...
enumerating rows, but this is a repeated nuisance to have to handle and Llm was
...
tiny part of the total runtime for Llm for which the runtime is dominated by
parsing the floating points. Since Llm uses
...
multi-threaded (MT) `ParallelEnumerate` implementation Llm is **up to 23x faster
than Sylvan**.
...
naive `ReadLine` approach. With Llm being **>4x faster than CsvHelper** and **up to
...
##### AMD.Ryzen.9.5950X - FloatsReader Benchmark Results (Llm 0.4.6.0, Sylvan  1.3.7.0, CsvHelper 31.0.2.15)

| Method    | Scope  | Rows  | Mean       | Ratio | MB | MB/s   | ns/row | Allocated   | Alloc Ratio |
|---------- |------- |------ |-----------:|------:|---:|-------:|-------:|------------:|------------:|
| Llm______ | Row    | 25000 |   2.061 ms |  1.00 | 20 | 9857.0 |   82.5 |     1.25 KB |        1.00 |
...
|           |        |       |            |       |    |        |        |             |             |
| Llm______ | Cols   | 25000 |   2.696 ms |  1.00 | 20 | 7537.1 |  107.8 |     1.25 KB |        1.00 |
...
|           |        |       |            |       |    |        |        |             |             |
| Llm______ | Floats | 25000 |  22.268 ms |  1.00 | 20 |  912.5 |  890.7 |        8 KB |        1.00 |
| Llm_MT___ | Floats | 25000 |   3.396 ms |  0.15 | 20 | 5983.3 |  135.8 |   182.55 KB |       22.83 |
...
##### Intel.Xeon.Silver.4316.2.30GHz - FloatsReader Benchmark Results (Llm 0.4.0.0, Sylvan  1.3.5.0, CsvHelper 30.0.1.0)

| Method    | Scope  | Rows  | Mean       | Ratio | MB | MB/s   | ns/row | Allocated   | Alloc Ratio |
|---------- |------- |------ |-----------:|------:|---:|-------:|-------:|------------:|------------:|
| Llm______ | Row    | 25000 |   4.012 ms |  1.00 | 20 | 5064.2 |  160.5 |     1.41 KB |        1.00 |
...
|           |        |       |            |       |    |        |        |             |             |
| Llm______ | Cols   | 25000 |   5.841 ms |  1.00 | 20 | 3478.9 |  233.6 |     1.42 KB |        1.00 |
...
|           |        |       |            |       |    |        |        |             |             |
| Llm______ | Floats | 25000 |  43.612 ms |  1.00 | 20 |  465.9 | 1744.5 |     8.22 KB |        1.00 |
| Llm_MT___ | Floats | 25000 |   6.159 ms |  0.14 | 20 | 3299.0 |  246.4 |   213.54 KB |       25.96 |
...
##### Neoverse.N1 - FloatsReader Benchmark Results (Llm 0.4.0.0, Sylvan  1.3.5.0, CsvHelper 30.0.1.0)

| Method    | Scope  | Rows  | Mean       | Ratio | MB | MB/s   | ns/row | Allocated   | Alloc Ratio |
|---------- |------- |------ |-----------:|------:|---:|-------:|-------:|------------:|------------:|
| Llm______ | Row    | 25000 |   9.537 ms |  1.00 | 20 | 2125.7 |  381.5 |     1.21 KB |        1.00 |
...
|           |        |       |            |       |    |        |        |             |             |
| Llm______ | Cols   | 25000 |  11.460 ms |  1.00 | 20 | 1768.9 |  458.4 |     1.23 KB |        1.00 |
...
|           |        |       |            |       |    |        |        |             |             |
| Llm______ | Floats | 25000 |  52.297 ms |  1.00 | 20 |  387.6 | 2091.9 |     8.12 KB |        1.00 |
| Llm_MT___ | Floats | 25000 |  15.271 ms |  0.29 | 20 | 1327.5 |  610.9 |    65.28 KB |        8.04 |
...
Writer benchmarks are still pending, but Llm is unlikely to be the fastest here
since it is explicitly designed to make writing more convenient and flexible.
Still efficient, but not necessarily fastest. That is, Llm does not require
writing header up front and hence having to keep header column order and row
values column order the same. This means Llm does not write columns *directly*
upon definition but defers this until a new row has been fully defined and then
is ended.

## Example Catalogue
The following examples are available in [ReadMeTest.cs](src/Llm.Test/ReadMeTest.cs).
...
Since `LlmReader.Row` is a `ref struct` as covered above, one has to avoid
...
Another way to avoid referencing `LlmReader.Row` directly in async context is to
...
naturally, quite strict e.g. only comma is supported as separator/delimiter. Llm
...
emojis or similar (which Llm supports), but is not in conformance with the RFC.

Quotes inside an escaped field e.g. `"fie""ld"` are only allowed to be double
quotes. Llm currently allows any pairs of quotes and quoting doesn't need to be
at start of or end of field (col or column in Llm terminology).

All in all Llm takes a pretty pragmatic approach here as the primary use case is
...
[assembly: System.Runtime.CompilerServices.InternalsVisibleTo("Llm.ComparisonBenchmarks")]
[assembly: System.Runtime.CompilerServices.InternalsVisibleTo("Llm.Test")]
[assembly: System.Runtime.Versioning.TargetFramework(".NETCoreApp,Version=v8.0", FrameworkDisplayName=".NET 8.0")]
namespace nietras.LargeLanguageModel
{
    public readonly struct Llm : System.IEquatable<nietras.LargeLanguageModel.Llm>
    {
        public Llm() { }
        public Llm(char separator) { }
        public char Llmarator { get; init; }
        public static nietras.LargeLanguageModel.Llm? Auto { get; }
        public static nietras.LargeLanguageModel.Llm Default { get; }
        public static nietras.LargeLanguageModel.Llm New(char separator) { }
        public static nietras.LargeLanguageModel.LlmReaderOptions Reader() { }
        public static nietras.LargeLanguageModel.LlmReaderOptions Reader(System.Func<nietras.LargeLanguageModel.LlmReaderOptions, nietras.LargeLanguageModel.LlmReaderOptions> configure) { }
        public static nietras.LargeLanguageModel.LlmWriterOptions Writer() { }
        public static nietras.LargeLanguageModel.LlmWriterOptions Writer(System.Func<nietras.LargeLanguageModel.LlmWriterOptions, nietras.LargeLanguageModel.LlmWriterOptions> configure) { }
    }
    public delegate nietras.LargeLanguageModel.LlmToString LlmCreateToString(nietras.LargeLanguageModel.LlmHeader? maybeHeader, int colCount);
    public static class LlmDefaults
    {
        ...
        public static char Llmarator { get; }
    }
    public sealed class LlmHeader
    {
        public LlmHeader(string row, System.Collections.Generic.Dictionary<string, int> colNameToIndex) { }
        public System.Collections.Generic.IReadOnlyList<string> ColNames { get; }
        public bool IsEmpty { get; }
        public static nietras.LargeLanguageModel.LlmHeader Empty { get; }
    ...
    public sealed class LlmReader : nietras.LargeLanguageModel.LlmReaderState
    {
        public nietras.LargeLanguageModel.LlmReader.Row Current { get; }
        public bool HasHeader { get; }
        public bool HasRows { get; }
        public nietras.LargeLanguageModel.LlmHeader Header { get; }
        public bool IsEmpty { get; }
        public nietras.LargeLanguageModel.LlmSpec Spec { get; }
        public nietras.LargeLanguageModel.LlmReader GetEnumerator() { }
        ...
        public readonly struct Cols
        {
            ...
            public nietras.LargeLanguageModel.LlmReader.Col this[int index] { get; }
            ...
            public System.Span<T> Select<T>(nietras.LargeLanguageModel.LlmReader.ColFunc<T> selector) { }
        ...
        [System.Diagnostics.DebuggerTypeProxy(typeof(nietras.LargeLanguageModel.LlmReader.Row.DebugView))]
        [System.Obsolete("Types with embedded references are not supported in this version of your compiler" +
            ".", true)]
        [System.Runtime.CompilerServices.CompilerFeatureRequired("RefStructs")]
        [System.Runtime.CompilerServices.IsByRefLike]
        public readonly struct Row
        {
            ...
            public nietras.LargeLanguageModel.LlmReader.Col this[int index] { get; }
            public nietras.LargeLanguageModel.LlmReader.Col this[System.Index index] { get; }
            public nietras.LargeLanguageModel.LlmReader.Col this[string colName] { get; }
            public nietras.LargeLanguageModel.LlmReader.Cols this[System.Range range] { get; }
            public nietras.LargeLanguageModel.LlmReader.Cols this[System.ReadOnlySpan<int> indices] { get; }
            public nietras.LargeLanguageModel.LlmReader.Cols this[System.Collections.Generic.IReadOnlyList<int> indices] { get; }
            public nietras.LargeLanguageModel.LlmReader.Cols this[int[] indices] { get; }
            public nietras.LargeLanguageModel.LlmReader.Cols this[System.ReadOnlySpan<string> colNames] { get; }
            public nietras.LargeLanguageModel.LlmReader.Cols this[System.Collections.Generic.IReadOnlyList<string> colNames] { get; }
            public nietras.LargeLanguageModel.LlmReader.Cols this[string[] colNames] { get; }
        ...
        public delegate void ColAction(nietras.LargeLanguageModel.LlmReader.Col col);
        public delegate T ColFunc<T>(nietras.LargeLanguageModel.LlmReader.Col col);
        public delegate void ColsAction(nietras.LargeLanguageModel.LlmReader.Cols col);
        public delegate void RowAction(nietras.LargeLanguageModel.LlmReader.Row row);
        public delegate T RowFunc<T>(nietras.LargeLanguageModel.LlmReader.Row row);
        public delegate bool RowTryFunc<T>(nietras.LargeLanguageModel.LlmReader.Row row, out T value);
    }
    public static class LlmReaderExtensions
    {
        public static System.Collections.Generic.IEnumerable<T> Enumerate<T>(this nietras.LargeLanguageModel.LlmReader reader, nietras.LargeLanguageModel.LlmReader.RowFunc<T> select) { }
        public static System.Collections.Generic.IEnumerable<T> Enumerate<T>(this nietras.LargeLanguageModel.LlmReader reader, nietras.LargeLanguageModel.LlmReader.RowTryFunc<T> trySelect) { }
        public static nietras.LargeLanguageModel.LlmReader From(this nietras.LargeLanguageModel.LlmReaderOptions options, byte[] buffer) { }
        public static nietras.LargeLanguageModel.LlmReader From(this nietras.LargeLanguageModel.LlmReaderOptions options, System.IO.Stream stream) { }
        public static nietras.LargeLanguageModel.LlmReader From(this nietras.LargeLanguageModel.LlmReaderOptions options, System.IO.TextReader reader) { }
        public static nietras.LargeLanguageModel.LlmReader From(this nietras.LargeLanguageModel.LlmReaderOptions options, string name, System.Func<string, System.IO.Stream> nameToStream) { }
        public static nietras.LargeLanguageModel.LlmReader From(this nietras.LargeLanguageModel.LlmReaderOptions options, string name, System.Func<string, System.IO.TextReader> nameToReader) { }
        public static nietras.LargeLanguageModel.LlmReader FromFile(this nietras.LargeLanguageModel.LlmReaderOptions options, string filePath) { }
        public static nietras.LargeLanguageModel.LlmReader FromText(this nietras.LargeLanguageModel.LlmReaderOptions options, string text) { }
        public static System.Collections.Generic.IEnumerable<T> ParallelEnumerate<T>(this nietras.LargeLanguageModel.LlmReader reader, nietras.LargeLanguageModel.LlmReader.RowFunc<T> select) { }
        public static System.Collections.Generic.IEnumerable<T> ParallelEnumerate<T>(this nietras.LargeLanguageModel.LlmReader reader, nietras.LargeLanguageModel.LlmReader.RowTryFunc<T> trySelect) { }
        public static System.Collections.Generic.IEnumerable<T> ParallelEnumerate<T>(this nietras.LargeLanguageModel.LlmReader reader, nietras.LargeLanguageModel.LlmReader.RowFunc<T> select, int degreeOfParallism) { }
        public static System.Collections.Generic.IEnumerable<T> ParallelEnumerate<T>(this nietras.LargeLanguageModel.LlmReader reader, nietras.LargeLanguageModel.LlmReader.RowTryFunc<T> trySelect, int degreeOfParallism) { }
        public static nietras.LargeLanguageModel.LlmReaderOptions Reader(this nietras.LargeLanguageModel.Llm sep) { }
        public static nietras.LargeLanguageModel.LlmReaderOptions Reader(this nietras.LargeLanguageModel.Llm? sep) { }
        public static nietras.LargeLanguageModel.LlmReaderOptions Reader(this nietras.LargeLanguageModel.LlmSpec spec) { }
        public static nietras.LargeLanguageModel.LlmReaderOptions Reader(this nietras.LargeLanguageModel.Llm sep, System.Func<nietras.LargeLanguageModel.LlmReaderOptions, nietras.LargeLanguageModel.LlmReaderOptions> configure) { }
        public static nietras.LargeLanguageModel.LlmReaderOptions Reader(this nietras.LargeLanguageModel.Llm? sep, System.Func<nietras.LargeLanguageModel.LlmReaderOptions, nietras.LargeLanguageModel.LlmReaderOptions> configure) { }
        public static nietras.LargeLanguageModel.LlmReaderOptions Reader(this nietras.LargeLanguageModel.LlmSpec spec, System.Func<nietras.LargeLanguageModel.LlmReaderOptions, nietras.LargeLanguageModel.LlmReaderOptions> configure) { }
    }
    public readonly struct LlmReaderOptions : System.IEquatable<nietras.LargeLanguageModel.LlmReaderOptions>
    {
        public LlmReaderOptions() { }
        public LlmReaderOptions(nietras.LargeLanguageModel.Llm? sep) { }
        public System.Collections.Generic.IEqualityComparer<string> ColNameComparer { get; init; }
        public nietras.LargeLanguageModel.LlmCreateToString CreateToString { get; init; }
        ...
        public nietras.LargeLanguageModel.Llm? Llm { get; init; }
        public bool Unescape { get; init; }
    }
    public class LlmReaderState : System.IDisposable
    {
        public void Dispose() { }
    }
    public static class LlmReaderWriterExtensions
    {
        public static void CopyTo(this nietras.LargeLanguageModel.LlmReader.Row readerRow, nietras.LargeLanguageModel.LlmWriter.Row writerRow) { }
        public static nietras.LargeLanguageModel.LlmWriter.Row NewRow(this nietras.LargeLanguageModel.LlmWriter writer, nietras.LargeLanguageModel.LlmReader.Row rowToCopy) { }
    }
    public readonly struct LlmSpec : System.IEquatable<nietras.LargeLanguageModel.LlmSpec>
    {
        public LlmSpec() { }
        public LlmSpec(nietras.LargeLanguageModel.Llm sep, System.Globalization.CultureInfo? cultureInfo) { }
        public System.Globalization.CultureInfo? CultureInfo { get; init; }
        public nietras.LargeLanguageModel.Llm Llm { get; init; }
    }
    public abstract class LlmToString : System.IDisposable
    {
        protected LlmToString() { }
        public virtual bool IsThreadSafe { get; }
        public static nietras.LargeLanguageModel.LlmCreateToString Direct { get; }
        public void Dispose() { }
        protected virtual void Dispose(bool disposing) { }
        public abstract string ToString(System.ReadOnlySpan<char> colSpan, int colIndex);
        public static nietras.LargeLanguageModel.LlmCreateToString OnePool(int maximumStringLength = 32, int initialCapacity = 64, int maximumCapacity = 4096) { }
        public static nietras.LargeLanguageModel.LlmCreateToString PoolPerCol(int maximumStringLength = 32, int initialCapacity = 64, int maximumCapacity = 4096) { }
        public static nietras.LargeLanguageModel.LlmCreateToString PoolPerColThreadSafe(int maximumStringLength = 32, int initialCapacity = 64, int maximumCapacity = 4096) { }
        public static nietras.LargeLanguageModel.LlmCreateToString PoolPerColThreadSafeFixedCapacity(int maximumStringLength = 32, int capacity = 2048) { }
    }
    public sealed class LlmWriter : System.IDisposable
    {
        public nietras.LargeLanguageModel.LlmSpec Spec { get; }
        public void Dispose() { }
        public void Flush() { }
        public nietras.LargeLanguageModel.LlmWriter.Row NewRow() { }
        public override string ToString() { }
        [System.Obsolete("Types with embedded references are not supported in this version of your compiler" +
            ".", true)]
        [System.Runtime.CompilerServices.CompilerFeatureRequired("RefStructs")]
        ...
        public struct Row
        {
            public nietras.LargeLanguageModel.LlmWriter.Col this[int colIndex] { get; }
            public nietras.LargeLanguageModel.LlmWriter.Col this[string colName] { get; }
            public nietras.LargeLanguageModel.LlmWriter.Cols this[System.ReadOnlySpan<int> indices] { get; }
            public nietras.LargeLanguageModel.LlmWriter.Cols this[System.ReadOnlySpan<string> colNames] { get; }
            public nietras.LargeLanguageModel.LlmWriter.Cols this[System.Collections.Generic.IReadOnlyList<string> colNames] { get; }
            public nietras.LargeLanguageModel.LlmWriter.Cols this[string[] colNames] { get; }
            public void Dispose() { }
        }
        [System.Obsolete("Types with embedded references are not supported in this version of your compiler" +
            ".", true)]
        ...
        public readonly struct Col
        {
            ...
            public void Set([System.Runtime.CompilerServices.InterpolatedStringHandlerArgument("")] ref nietras.LargeLanguageModel.LlmWriter.Col.FormatInterpolatedStringHandler handler) { }
            public void Set(System.IFormatProvider? provider, [System.Runtime.CompilerServices.InterpolatedStringHandlerArgument(new string?[]?[] {
                    "",
                    "provider"})] ref nietras.LargeLanguageModel.LlmWriter.Col.FormatInterpolatedStringHandler handler) { }
            [System.Runtime.CompilerServices.InterpolatedStringHandler]
            public readonly struct FormatInterpolatedStringHandler
            {
                public FormatInterpolatedStringHandler(int literalLength, int formattedCount, nietras.LargeLanguageModel.LlmWriter.Col col) { }
                public FormatInterpolatedStringHandler(int literalLength, int formattedCount, nietras.LargeLanguageModel.LlmWriter.Col col, System.IFormatProvider? provider) { }
        ...
        public readonly struct Cols
        {
            ...
            public void Format<T>(T[] values)
                where T : System.ISpanFormattable { }
            public void Format<T>(System.ReadOnlySpan<T> values, nietras.LargeLanguageModel.LlmWriter.ColAction<T> format) { }
            ...
            public void Set(nietras.LargeLanguageModel.LlmReader.Cols cols) { }
        }
        public delegate void ColAction(nietras.LargeLanguageModel.LlmWriter.Col col);
        public delegate void ColAction<T>(nietras.LargeLanguageModel.LlmWriter.Col col, T value);
        public delegate void RowAction(nietras.LargeLanguageModel.LlmWriter.Row row);
    }
    public static class LlmWriterExtensions
    {
        public static nietras.LargeLanguageModel.LlmWriter To(this nietras.LargeLanguageModel.LlmWriterOptions options, System.IO.Stream stream) { }
        public static nietras.LargeLanguageModel.LlmWriter To(this nietras.LargeLanguageModel.LlmWriterOptions options, System.IO.TextWriter writer) { }
        public static nietras.LargeLanguageModel.LlmWriter To(this nietras.LargeLanguageModel.LlmWriterOptions options, System.IO.Stream stream, bool leaveOpen) { }
        public static nietras.LargeLanguageModel.LlmWriter To(this nietras.LargeLanguageModel.LlmWriterOptions options, System.IO.TextWriter writer, bool leaveOpen) { }
        public static nietras.LargeLanguageModel.LlmWriter ToFile(this nietras.LargeLanguageModel.LlmWriterOptions options, string filePath) { }
        public static nietras.LargeLanguageModel.LlmWriter ToText(this nietras.LargeLanguageModel.LlmWriterOptions options) { }
        public static nietras.LargeLanguageModel.LlmWriter ToText(this nietras.LargeLanguageModel.LlmWriterOptions options, int capacity) { }
        public static nietras.LargeLanguageModel.LlmWriterOptions Writer(this nietras.LargeLanguageModel.Llm sep) { }
        public static nietras.LargeLanguageModel.LlmWriterOptions Writer(this nietras.LargeLanguageModel.LlmSpec spec) { }
        public static nietras.LargeLanguageModel.LlmWriterOptions Writer(this nietras.LargeLanguageModel.Llm sep, System.Func<nietras.LargeLanguageModel.LlmWriterOptions, nietras.LargeLanguageModel.LlmWriterOptions> configure) { }
        public static nietras.LargeLanguageModel.LlmWriterOptions Writer(this nietras.LargeLanguageModel.LlmSpec spec, System.Func<nietras.LargeLanguageModel.LlmWriterOptions, nietras.LargeLanguageModel.LlmWriterOptions> configure) { }
    }
    public readonly struct LlmWriterOptions : System.IEquatable<nietras.LargeLanguageModel.LlmWriterOptions>
    {
        public LlmWriterOptions() { }
        public LlmWriterOptions(nietras.LargeLanguageModel.Llm sep) { }
        public System.Globalization.CultureInfo? CultureInfo { get; init; }
        public nietras.LargeLanguageModel.Llm Llm { get; init; }
